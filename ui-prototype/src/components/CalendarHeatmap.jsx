...
// (непрерывная альфа с полом 0.35 — каждый торговый день виден). Тянется на всю ширину.
// Порт боевого CalendarHeatmap, нейтральные цвета адаптированы под тему (light-dark).
...
const EMPTY = 'light-dark(var(--mantine-color-gray-2), var(--mantine-color-dark-5))'
const ZERO = 'light-dark(var(--mantine-color-gray-3), var(--mantine-color-dark-4))'
...
export default function CalendarHeatmap({ data = {}, days = 364, gap = 3 }) {
  ...
  let maxAbs = 0
  Object.values(data).forEach((v) => { if (typeof v === 'number' && Math.abs(v) > maxAbs) maxAbs = Math.abs(v) })
